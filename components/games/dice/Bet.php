<div class="w-full dice-bet translate-y-[1rem] opacity-0 py-4 bg-gradient-to-b from-transparent to-[#161823] min-[1125px]:to-[70%] to-[20%] min-[861px]:flex grid grid-cols-2 items-stretch justify-between px-4 gap-4 z-[1]">
    ...
    <div class="flex items-stretch gap-2 max-[650px]:col-span-2" x-data="{ edit: false, }">
        ...
        <div class="flex flex-wrap gap-2 max-w-[13.75rem]">
            <button type="button" aria-label="10" role="button" @click="chance = 10" x-bind:class="chance === 10 ? 'active' : ''" class="h-[2rem] w-[calc(50%-0.25rem)] flex items-center font-semibold text-sm justify-center px-2 rounded-lg bg-[#2a2b3c] [&.active]:bg-blue transition-all duration-300 hover:bg-blue">10</button>
            <button type="button" aria-label="25" role="button" @click="chance = 25" x-bind:class="chance === 25 ? 'active' : ''" class="h-[2rem] w-[calc(50%-0.25rem)] flex items-center font-semibold text-sm justify-center px-2 rounded-lg bg-[#2a2b3c] [&.active]:bg-blue transition-all duration-300 hover:bg-blue">25</button>
            <button type="button" aria-label="50" role="button" @click="chance = 50" x-bind:class="chance === 50 ? 'active' : ''" class="h-[2rem] w-[calc(50%-0.25rem)] flex items-center font-semibold text-sm justify-center px-2 rounded-lg bg-[#2a2b3c] [&.active]:bg-blue transition-all duration-300 hover:bg-blue">50</button>
            <button type="button" aria-label="75" role="button" @click="chance = 75" x-bind:class="chance === 75 ? 'active' : ''" class="h-[2rem] w-[calc(50%-0.25rem)] flex items-center font-semibold text-sm justify-center px-2 rounded-lg bg-[#2a2b3c] [&.active]:bg-blue transition-all duration-300 hover:bg-blue">75</button>
        </div>
    </div>
    <button aria-label="Меньше" role="button" :disabled="loader" x-data="btn" @click="eventClick(200); setTimeout(() => { play('min') }, 100)" class="min-h-[3.438rem] flex-1 uppercase relative font-[600] flex rounded-[.375rem] items-center justify-center text-white px-[1.75rem] transition-[background_,_box-shadow] duration-300 hover:bg-[#fb3a3a] bg-[#e72a2a]">
        <span class="transition-[opacity] opacity-100" x-bind:class="loader ? '!opacity-0' : ''">Меньше</span>
    ...
    <button aria-label="Больше" role="button" :disabled="loader" x-data="btn" @click="eventClick(200); setTimeout(() => { play('max')  }, 100)" class="min-h-[3.438rem] flex-1 uppercase relative font-[600] flex rounded-[.375rem] items-center justify-center text-white px-[1.75rem] transition-[background_,_box-shadow] duration-300 hover:bg-[#2399EF] bg-blue hover:shadow-btn-blue">
        <span class="transition-[opacity] opacity-100" x-bind:class="loader ? '!opacity-0' : ''">Больше</span>
        <div class="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 opacity-0 transition-[opacity]" x-bind:class="loader ? '!opacity-100' : ''">
            <?php include __DIR__.'/../../ui/Loader.php' ?>
        </div>
    </button>
